...
*/
#include <sql_server_database.hpp>
#include <sql_server_field.hpp>
#include <sql_server_table_impl.hpp>
//-----------------------------------------------------------------------------------------------//
namespace sql::server
{
	//-----------------------------------------------------------------------------------------------//
	table_impl::table_impl() :server_connection(), std::map<key_type, mapped_type>()
	{
		if(this->CreateInstance(__uuidof(Recordset)) != S_OK)
		{
			throw std::runtime_error("Failed to create object Recordset!");
		}
	}
	//-----------------------------------------------------------------------------------------------//
